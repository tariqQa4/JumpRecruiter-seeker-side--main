class ApplyJob {
  ...
  clickOnSpecificjob() {
    ...
    cy.wait(2000);
    cy.get('button:has(svg path[d="m9 18 6-6-6-6"])').click({ force: true })
    cy.wait(2000);
    cy.scrollTo("top", { duration: 3000 });
    cy.wait(2000);
    cy.get("div.group")
      .filter(':has(button:contains("Easy Apply"))')
      .first()
      .within(() => {
        cy.contains("button", /easy apply/i).click();
      });
  ...
  applyNow() {
    //apply now button
    cy.contains('button', 'APPLY NOW').first().click().should("be.visible")
    .then(($input) => {
      $input.css("border", "4px solid red");
    });
